...
/**
 * Shared per-customer synchronization logic.
 *
 * This holds the role/DB reconciliation that used to live inline in permsCheck.
 * It is now reused by:
 *   - permsCheck (cron mode): iterates every customer and calls this for each one.
 *   - the Stripe webhook server (webhook mode): calls this for the single customer
 *     affected by an incoming event.
 *
 * Keeping it in one place guarantees cron and webhook produce identical results.
 */

// Remove roles from a member based on plan configuration
...
exports.removeRolesFromMember = removeRolesFromMember;
...
/**
 * Reconcile a single customer's Discord roles against their current Stripe state.
 *
 * @param {Object} client     - Discord client instance
 * @param {Object} customer    - Customer document from the database (needs email + discordId)
 * @param {Object} [ctx]       - Optional shared context to avoid redundant lookups
 * @param {Object} [ctx.guild]      - Pre-resolved guild (cron passes this once for all customers)
 * @param {Object} [ctx.collection] - Pre-resolved DB collection
 * @returns {Promise<void>}
 */
module.exports = async function syncCustomer(client, customer, ctx = {}) {
    if (!customer || !customer.email) return;

    const guild = ctx.guild || client.guilds.cache.get(process.env.GUILD_ID);
    if (!guild) {
        console.error('[Account Verification] Could not find guild');
        return;
    }
    ...
    if (!collection) {
        ...
        collection = discordDB.collection(process.env.DATABASE_COLLECTION_NAME);
    }
    ...
    if (!updatedCustomer || !updatedCustomer.activeSubscribed) {
    ...
    if ((activeSubscriptions.length === 0 || allSubscriptions.every(sub => sub.status === 'unpaid')) && customer.activeSubscribed) {
        ...
        await makeMemberExpire(customer, member, guild, collection);
        return;
    }
